...
labs(title = "European homicide trends", x = "Year", y = "Homicide rate", colour = "European regions")
```
...
```{r homicides-per-country, eval = TRUE}
  #... YOUR CODE
ggplot(data = Western_Europe, aes(x = Year, y = homicides_per_100k))+
geom_line(aes(group = Entity, colour = factor(Entity)))+
...
```{r Danish-kings, eval = TRUE}
# YOUR CODE HERE:
library(tidyverse)
library(here)
monarchs <- read_delim(here("data/monarchs_clean.csv")) 
monarchs_mid <- monarchs %>% 
...
monarchs_mid %>% 
  ...
  geom_smooth()+
  labs(titel = "Timeline for monarchs", x="Middle of reign", y="Duration of reign")
```
  Answer to question posed above
As the homicide rates drop, the duration of reign for the rulers increase. 
...
3) Question: In <250 words articulate your answer on the basis of the data visualisations to the following question: are we more civilized today?
  ...
  Answer
The number of homicides have gown down over the years, so that might indicate that we have become more civilized over time. The reign of kings have also become longer, and that might be because they stopped killing kings during conflicts. It might also have something to do with the fact that livingstandards have improved since. But you cannot conclude wether or not we are more civilized today from the data visualizations we have made, since there can be multiple reasons why the data is the way it is.  